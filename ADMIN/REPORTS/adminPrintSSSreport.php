...
while ($check1array = mysqli_fetch_array($checkpayperperiodexec)):;
  ...
  $getsssinfoquery = "SELECT gsisEE,gsisER,gsisTOTAL FROM PAYROLLINFO WHERE emp_id = '$empid'";
  ...
  if($sssinfoarray){
    $gsisEE = $sssinfoarray['gsisEE'];
    $gsisER = $sssinfoarray['gsisER'];
    $gsisTOTAL = $sssinfoarray['gsisTOTAL'];

    $eetot = $eetotal + $gsisEE;
    $ertot = $ertotal + $gsisER;
    $total = $total + $gsisTOTAL;
    $gsistot = number_format((float)$total,2,'.','');
    $eetotal = number_format((float)$eetot,2,'.','');
    $ertotal = number_format((float)$ertot,2,'.','');
  }

  $getdetailsquery = "SELECT last_name,first_name,middle_name,GSIS_idno FROM employees WHERE emp_id = '$empid'";
  ...
  if($getdetailsarray){
    ...
    $gsisidno = $getdetailsarray['GSIS_idno'];
...
$pdf->Cell(44.25,7,$gsisidno,1,0,'C');
$pdf->Cell(23,7,$fname,1,0,'C');
$pdf->Cell(23,7,$lname,1,0,'C');
$pdf->Cell(23,7,$mname,1,0,'C');
$pdf->Cell(25,7,$gsisER,1,0,'C');
$pdf->Cell(25,7,$gsisEE,1,0,'C');
$pdf->Cell(34,7,$gsisTOTAL,1,1,'C');//end
