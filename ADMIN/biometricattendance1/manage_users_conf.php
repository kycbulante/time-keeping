...
if (isset($_POST['Add_fingerID'])) {
    ...
    if ($fingerid == 0) {
        ...
        if ($fingerid > 0 && $fingerid < 128) {
            $sql = "SELECT fingerprint_id FROM fingerprint WHERE fingerprint_id=?";
            ...
            if (!mysqli_stmt_prepare($result, $sql)) {
              ...
              exit();
                ...
                if (!$row = mysqli_fetch_assoc($resultl)) {
                    ...
                    $sql = "SELECT add_fingerid FROM fingerprint WHERE add_fingerid=1";
                    $result = mysqli_stmt_init($conn1);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                      echo "SQL_Error2";
                      exit();
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql = "INSERT INTO fingerprint (fingerprint_id, emp_id, user_name, add_fingerid) VALUES (?, ?, ?, 1)";
                            $result = mysqli_stmt_init($conn1);
                            $sql1="UPDATE employees SET fingerprint_id = 1 WHERE emp_id = '$emp_id'";
                            $result_update = mysqli_query($conn1, $sql1);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                              echo "SQL_Error3";
                              exit();
                            }
                            else{
                                mysqli_stmt_bind_param($result, "iis", $fingerid, $emp_id, $user_name);
                                mysqli_stmt_execute($result);
