...
if (isset($_POST['searchbydate_btn'])){
  ...
  $searchquery = "SELECT OVER_TIME.*, employees.* from employees, OVER_TIME  WHERE OVER_TIME.ot_day = '$datesearch' AND OVER_TIME.ot_remarks = 'Approved' AND employees.emp_id = OVER_TIME.emp_id and employees.emp_id = '$currentempid' ORDER BY OVER_TIME.ot_day DESC LIMIT $start_from,".$results_perpage;  
  ...
  $start_from = ($page-1) * $results_perpage;
  // $datesearch = $_POST['dphired'];
  $searchquery = "SELECT OVER_TIME.*, employees.* from employees, OVER_TIME  WHERE OVER_TIME.ot_remarks = 'Approved' AND employees.emp_id = OVER_TIME.emp_id and employees.emp_id = '$currentempid' ORDER BY OVER_TIME.ot_day DESC LIMIT $start_from,".$results_perpage;  
  $search_result = filterTable($searchquery);


}

$countdataqry = "SELECT COUNT(ot_ID) AS total FROM OVER_TIME WHERE ot_remarks ='Approved' AND emp_id = '$currentempid'";
        ...
        <div class="widget-box">
          <div class="widget-title">
            <ul class="nav nav-tabs" id="myTab">
              ...
              <li class="active"><a href="empOTApproved.php"><i class="icon-time"></i> Approved</a></li>
