...
// Assuming $_SESSION['printtimesheet_query'] contains the stored query


// Execute the stored query and fetch results
...
$storedQuery = "SELECT * FROM employees JOIN pay_per_period ON employees.emp_id = pay_per_period.emp_id WHERE pay_per_period.pperiod_range = '$payperiod' AND employees.emp_id='$empid' ORDER BY PAY_PER_PERIOD.emp_id ASC";
...
while ($employee = mysqli_fetch_assoc($employeeResult)) {
   ...
   $printquery = "SELECT * FROM TIME_KEEPING, employees 
               ...
               AND TIME_KEEPING.timekeep_day BETWEEN '$period_start' AND '$period_end' 
...
$payperiodval = "SELECT undertime_hours,rh_hours,late_hours,timekeep_day,overtime_hours,hours_work,timekeep_remarks,(hours_work+overtime_hours) as totalhours FROM TIME_KEEPING WHERE emp_id = '$printid'  AND TIME_KEEPING.timekeep_day BETWEEN '$period_start' AND '$period_end' ORDER BY timekeep_day ASC";
$payperiodexec = mysqli_query($conn,$payperiodval) or die ("FAILED TO QUERY TIMEKEEP DETAILS ".mysqli_error($conn));

$totalot = "SELECT SUM(undertime_hours) as totalUT ,SUM(overtime_hours) as totalOT, SUM(hours_work) as totalWORKhours, SUM(late_hours) as totalLATEhours, SUM(hours_work+overtime_hours) as totalness FROM TIME_KEEPING WHERE emp_id = '$printid' AND timekeep_day  AND TIME_KEEPING.timekeep_day BETWEEN '$period_start' AND '$period_end' ORDER BY timekeep_day ASC";
$totalotexec =mysqli_query($conn,$totalot) or die ("OT ERROR ".mysqli_error($conn));
$totalotres = mysqli_fetch_array($totalotexec);

$absencesqry = "SELECT ('$daycount'-COUNT(timekeep_day)) as absences FROM TIME_KEEPING WHERE emp_id = '$printid' AND timekeep_day AND TIME_KEEPING.timekeep_day BETWEEN '$period_start' AND '$period_end' ORDER BY timekeep_day ASC";
...
$pdf->Cell(40, 5, $period_start, 0, 1);  // end of line
$pdf->Cell(59, 5, $period_end , 0, 0);
